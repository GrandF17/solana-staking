...
//       await program.provider.connection.getMinimumBalanceForRentExemption(MINT_SIZE)
...
//       .mintToken(new BN(10_000_000_000))
//       .accounts({
...
//     const minted = (
//       await program.provider.connection.getParsedAccountInfo(associatedTokenAccount)
...
//     await program.methods
//       .transferToken(new BN(1_000_000))
//       .accounts({
//         tokenProgram: TOKEN_PROGRAM_ID,
...
//       await program.provider.connection.getParsedAccountInfo(associatedTokenAccount)
//     ).value.data.parsed.info.tokenAmount.amount
//     assert.equal(minted, new BN(9_999_000_000))
//   })
...
describe('token-contract', () => {
  // create a user and mint tokens on our token_account
  const program = anchor.workspace.TokenContract as Program<TokenContract>
  const mintKey: anchor.web3.Keypair = anchor.web3.Keypair.generate()
})
